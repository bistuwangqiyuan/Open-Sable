...
pnpm -F @opensable/web test                    # Run all web Vitest tests
pnpm -F @opensable/web test:unit               # Web unit tests only
pnpm -F @opensable/web test:integration        # Web integration tests only

# Testing (desktop workspace — main process + preload tests)
pnpm -F @opensable/desktop test                # Run all desktop Vitest tests
pnpm -F @opensable/desktop test:unit           # Desktop unit tests only
pnpm -F @opensable/desktop test:integration    # Desktop integration tests only
pnpm -F @opensable/desktop test:e2e            # Docker-based E2E tests
pnpm -F @opensable/desktop test:e2e:native     # Native Playwright E2E tests (serial, Electron requirement)

# Testing (agent-core)
pnpm -F @opensable/agent-core test          # Run agent-core Vitest tests
...
pnpm -F @opensable/web test

# After changing desktop main process or preload code
pnpm -F @opensable/desktop test

# After changing agent-core code
pnpm -F @opensable/agent-core test

# Full verification before PR
pnpm lint && pnpm format:check && pnpm -F @opensable/web test && pnpm -F @opensable/desktop test && pnpm -F @opensable/agent-core test
```
...
- **Do NOT add root-level test scripts** — tests are workspace-scoped (`-F @opensable/web`, `-F @opensable/desktop`, or `-F @opensable/agent-core`)
...
- `@opensable/agent-core` — Core business logic, types, storage, MCP tools (ESM, internal workspace package)
- `@opensable/web` — Standalone React UI (Vite + React Router + Zustand)
- `@opensable/desktop` — Thin Electron shell (main process + preload), loads web's build output

## Code Conventions

- TypeScript everywhere (no JS for app logic)
- **ESM package**: `@opensable/agent-core` uses `"type": "module"` — all imports MUST use `.js` extensions
...
- IPC handlers in `apps/desktop/src/main/ipc/handlers.ts` must match `window.opensable` API in preload
...
3. Add the typed wrapper in `apps/web/src/client/lib/opensable.ts`
...
4. Run `pnpm -F @opensable/agent-core test`
...
```typescript
"@/*"                              → "src/client/*"
"@opensable/agent-core"        → "../../packages/agent-core/src/index.ts"
"@opensable/agent-core/*"      → "../../packages/agent-core/src/*"
"@opensable/agent-core/common" → "../../packages/agent-core/src/common.ts"
```
...
- `OPENSABLE_BUNDLED_MCP=1` - Bundle MCP tools in packaged build (used in package/release scripts)
