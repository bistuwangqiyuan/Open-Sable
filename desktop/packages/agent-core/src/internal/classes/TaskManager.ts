...
import { OpenSableAdapter, createOpenSableAdapter, type OpenSableConfig } from './OpenSableAdapter.js';
import type {
  TaskConfig,
  Task,
  TaskMessage,
...
export type AdapterMode = 'opencode' | 'opensable';
...
export interface TaskManagerOptions {
  ...
  /** Set to 'opensable' to connect via WebSocket to the OpenSable gateway */
  adapterMode?: AdapterMode;
  /** Config for OpenSable gateway connection (only used when adapterMode='opensable') */
  opensableConfig?: Partial<OpenSableConfig>;
}

interface ManagedTask {
  taskId: string;
  adapter: OpenCodeAdapter | OpenSableAdapter;
...
export class TaskManager {
  ...
  async startTask(taskId: string, config: TaskConfig, callbacks: TaskCallbacks): Promise<Task> {
    // In opensable mode, skip CLI availability check
    if (this.options.adapterMode !== 'opensable') {
  ...
  private async executeTask(
    ...
    let adapter: OpenCodeAdapter | OpenSableAdapter;

    if (this.options.adapterMode === 'opensable') {
      adapter = createOpenSableAdapter(this.options.opensableConfig);
    } else {
      const adapterOptions: AdapterOptions = {
        ...this.options.adapterOptions,
        buildCliArgs: (taskConfig) => this.options.adapterOptions.buildCliArgs(taskConfig, taskId),
      };
      adapter = new OpenCodeAdapter(adapterOptions, taskId);
    }
  ...
  getTask(taskId: string): OpenCodeAdapter | OpenSableAdapter | undefined {
