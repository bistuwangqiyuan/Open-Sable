...
export const OPENSABLE_AGENT_NAME = 'opensable';
...
const OPENSABLE_SYSTEM_PROMPT_TEMPLATE = `<identity>
You are OpenSable, a {{AGENT_ROLE}} assistant.
</identity>
...
- goal: What you aim to opensable
  ...
  if (!fs.existsSync(distPath)) {
    throw new Error(
      `[OpenCode Config] Missing MCP dist entry: ${distPath}. ` +
        'Run "pnpm -F @opensable/desktop build:mcp-tools:dev" before launching.',
    );
  }
...
export function generateConfig(options: ConfigGeneratorOptions): GeneratedConfig {
  ...
  let systemPrompt = OPENSABLE_SYSTEM_PROMPT_TEMPLATE.replace(
  ...
  if (!bundledNodeBinPath) {
    throw new Error(
      '[OpenCode Config] Missing bundled Node.js path; cannot launch MCP tools. ' +
        'Run "pnpm -F @opensable/desktop download:nodejs" and rebuild artifacts.',
    );
  }
...
`
  ...
  const config: OpenCodeConfigFile = {
    ...
    default_agent: OPENSABLE_AGENT_NAME,
...
export function buildCliArgs(options: BuildCliArgsOptions): string[] {
  ...
  args.push('--agent', OPENSABLE_AGENT_NAME);
