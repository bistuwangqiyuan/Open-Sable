...
const THOUGHT_STREAM_TASK_ID =
  process.env.THOUGHT_STREAM_TASK_ID || process.env.OPENSABLE_TASK_ID || '';
  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          summary: {
            type: 'string',
            description: 'Brief summary of what was opensableed or the current state',
          },
          nextPlanned: {
            type: 'string',
    ...
    try {
      const response = await fetch(CHECKPOINT_URL, {
        ...
        body: JSON.stringify({
          ...
          nextPlanned,
          blocker,
          agentName: process.env.OPENSABLE_AGENT_NAME || 'agent',
          timestamp: Date.now(),
        }),
