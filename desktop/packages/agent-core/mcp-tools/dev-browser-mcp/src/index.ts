...
console.error(
  '[dev-browser-mcp] OPENSABLE_TASK_ID:',
  process.env.OPENSABLE_TASK_ID || '(not set)',
);
...
async function loadToolDebug(): Promise<void> {
  const debugPath = process.env.OPENSABLE_TOOL_DEBUG_PATH;
  if (debugPath) {
    ...
    console.error('[dev-browser-mcp] OPENSABLE_TOOL_DEBUG_PATH not set, tool debug disabled');
  }
}
await loadToolDebug();
