...
describe('connection', () => {
  beforeEach(() => {
    ...
    delete process.env.OPENSABLE_TASK_ID;
  });
  ...
  describe('configureFromEnv', () => {
    ...
    it('uses OPENSABLE_TASK_ID for task isolation', () => {
      process.env.OPENSABLE_TASK_ID = 'task-abc';
      const cfg = configureFromEnv();
      expect(cfg.taskId).toBe('task-abc');
    });

    it('defaults taskId to "default" when OPENSABLE_TASK_ID is not set', () => {
      const cfg = configureFromEnv();
  ...
  describe('getFullPageName', () => {
    ...
    it('uses configureFromEnv taskId', () => {
      process.env.OPENSABLE_TASK_ID = 'env-task';
      configureFromEnv();
