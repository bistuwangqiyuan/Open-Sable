...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'OpenSable', 'dev-browser');
  } else if (process.platform === 'win32') {
    return join(process.env.APPDATA || homeDir, 'OpenSable', 'dev-browser');
  } else {
    return join(homeDir, '.opensable', 'dev-browser');
  }
}
...
const OPENSABLE_HTTP_PORT = parseInt(process.env.DEV_BROWSER_PORT || '9224', 10);
const OPENSABLE_CDP_PORT = parseInt(process.env.DEV_BROWSER_CDP_PORT || '9225', 10);

if (
  !Number.isFinite(OPENSABLE_HTTP_PORT) ||
  OPENSABLE_HTTP_PORT < 1 ||
  OPENSABLE_HTTP_PORT > 65535
) {
  throw new Error(
...
if (
  !Number.isFinite(OPENSABLE_CDP_PORT) ||
  OPENSABLE_CDP_PORT < 1 ||
  OPENSABLE_CDP_PORT > 65535
) {
  throw new Error(
...
try {
  const res = await fetch(`http://localhost:${OPENSABLE_HTTP_PORT}`, {
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          const output = execSync(`netstat -ano | findstr :${OPENSABLE_HTTP_PORT}`, {
            encoding: 'utf-8',
          });
          ...
          const pid = execSync(`lsof -ti:${OPENSABLE_HTTP_PORT}`, { encoding: 'utf-8' }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
          }
        }
      ...
      console.log(`Launch server already running on port ${OPENSABLE_HTTP_PORT}`);
      process.exit(0);
    }
  }
} catch {
...
try {
  if (process.platform === 'win32') {
    const output = execSync(`netstat -ano | findstr :${OPENSABLE_CDP_PORT}`, {
      encoding: 'utf-8',
    });
    const match = output.match(/LISTENING\s+(\d+)/);
    if (match) {
      const pid = match[1];
      console.log(
        `Cleaning up stale Chrome process on CDP port ${OPENSABLE_CDP_PORT} (PID: ${pid})`,
      );
      execSync(`taskkill /F /PID ${pid}`, { stdio: 'ignore' });
    }
  } else {
    const pid = execSync(`lsof -ti:${OPENSABLE_CDP_PORT}`, { encoding: 'utf-8' }).trim();
    if (pid) {
      console.log(
        `Cleaning up stale Chrome process on CDP port ${OPENSABLE_CDP_PORT} (PID: ${pid})`,
      );
...
async function startServer(retry = false): Promise<void> {
  try {
    const server = await serve({
      port: OPENSABLE_HTTP_PORT,
      cdpPort: OPENSABLE_CDP_PORT,
      headless,
      profileDir,
