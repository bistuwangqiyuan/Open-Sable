...
if (!process.env.NODE_BIN_PATH) {
  ...
  log('Run "pnpm -F @opensable/desktop download:nodejs" and rebuild before launching.');
  process.exit(1);
}
...
if (!fs.existsSync(bundledServer)) {
  ...
  log('Run "pnpm -F @opensable/desktop build:mcp-tools:dev" before starting the app.');
  process.exit(1);
}
