# @opensable/agent-core

Core library for the OpenSable desktop automation assistant. Provides factory-based APIs for task management, persistent storage, permission handling, logging, and more.

## Usage

This package is internal to the OpenSable monorepo and is consumed via workspace dependencies (for example, `"@opensable/agent-core": "workspace:*"`).

## Quick Start

```typescript
import { createStorage, createTaskManager } from '@opensable/agent-core';

// Initialize storage (SQLite + encrypted secure storage)
const storage = createStorage({
  databasePath: '/path/to/opensable.db',
...
```typescript
import { createTaskManager } from '@opensable/agent-core';
```
...
```typescript
import { createStorage } from '@opensable/agent-core';
```
...
```typescript
import { createPermissionHandler } from '@opensable/agent-core';
```
...
```typescript
import { createThoughtStreamHandler } from '@opensable/agent-core';
```
...
```typescript
import { createLogWriter } from '@opensable/agent-core';
```
...
```typescript
import { createSkillsManager } from '@opensable/agent-core';
```
...
```typescript
import { createSpeechService } from '@opensable/agent-core';
```
...
```typescript
import { generateConfig, type BrowserConfig } from '@opensable/agent-core';
