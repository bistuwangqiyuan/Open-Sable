...
export function AddSkillDropdown({ onSkillAdded, onClose }: AddSkillDropdownProps) {
  ...
  const handleUploadSkill = async () => {
    ...
    try {
      setIsLoading(true);
      setUploadError(null);
      const filePath = await window.opensable.pickSkillFile();
      if (!filePath) {
        setIsLoading(false);
        return; // User cancelled
      }
      await window.opensable.addSkillFromFile(filePath);
      onSkillAdded?.();
    } catch (err) {
  ...
  const handleImportFromGitHub = async () => {
    if (!gitHubUrl.trim() || !window.opensable) return;

    try {
      setIsLoading(true);
      setError(null);
      await window.opensable.addSkillFromGitHub(gitHubUrl);
      setGitHubUrl('');
        ...
        <DropdownMenuContent align="end" className="w-60">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{t('skills.buildWithOpenSable')}</span>
            </div>
