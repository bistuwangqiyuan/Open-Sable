...
import type { Skill } from '@opensable/agent-core/common';
...
export function SkillsPanel({ refreshTrigger }: SkillsPanelProps) {
  ...
  useEffect(() => {
    if (!window.opensable) {
      console.error('OpenSable API not available');
      setLoading(false);
      return;
    }
    window.opensable
  ...
  const handleToggle = useCallback(
    async (id: string) => {
      ...
      if (!skill || !window.opensable) return;

      try {
        await window.opensable.setSkillEnabled(id, !skill.isEnabled);
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        await window.opensable.deleteSkill(id);
  ...
  const handleEdit = useCallback(async (filePath: string) => {
    if (!window.opensable) return;
    try {
      await window.opensable.openSkillInEditor(filePath);
    } catch (err) {
  ...
  const handleShowInFolder = useCallback(async (filePath: string) => {
    if (!window.opensable) return;
    try {
      await window.opensable.showSkillInFolder(filePath);
    } catch (err) {
