...
export function DebugSection({ debugMode, onDebugToggle }: DebugSectionProps) {
  ...
  const handleExportLogs = useCallback(async () => {
    ...
    try {
      const result = await opensable.exportLogs();
      if (result.success) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="ml-4 flex items-center gap-3">
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ease-opensable ${
              debugMode ? 'bg-primary' : 'bg-muted'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ease-opensable ${
