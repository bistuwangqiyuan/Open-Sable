...
import type { ConnectedProvider, AzureFoundryCredentials } from '@opensable/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function AzureFoundryProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate connection
      const validation = await opensable.testAzureFoundryConnection({
      ...
      // Save credentials
      await opensable.saveAzureFoundryConfig({
        endpoint: endpoint.trim(),
        deploymentName: deploymentName.trim(),
        authType,
        apiKey: authType === 'api-key' ? apiKey.trim() : undefined,
