...
import { getDefaultModelForProvider } from '@opensable/agent-core/common';
import {
  ModelSelector,
  RegionSelector,
  ConnectButton,
...
export function BedrockProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await opensable.validateBedrockCredentials(credentials);
      ...
      // Save credentials
      await opensable.saveBedrockCredentials(credentials);

      // Fetch available models dynamically from AWS
      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await opensable.fetchBedrockModels(credentialsJson);
