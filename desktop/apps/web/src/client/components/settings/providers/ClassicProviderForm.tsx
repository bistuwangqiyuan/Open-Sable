...
import { PROVIDER_META, DEFAULT_PROVIDERS } from '@opensable/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function ClassicProviderForm({
  ...
  useEffect(() => {
    ...
    opensable.getOpenAiBaseUrl().then(setOpenAiBaseUrl).catch(console.error);
  }, [isOpenAI]);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (isOpenAI) {
        await opensable.setOpenAiBaseUrl(openAiBaseUrl.trim());
      }

      const validation = await opensable.validateApiKeyForProvider(providerId, apiKey.trim());
      ...
      await opensable.addApiKey(providerId as any, apiKey.trim());

      // Fetch models dynamically if provider has a models endpoint
      let fetchedModels: Array<{ id: string; name: string }> | undefined;
      if (providerConfig?.modelsEndpoint) {
        const fetchResult = await opensable.fetchProviderModels(providerId, {
          baseUrl: isOpenAI ? openAiBaseUrl.trim() || undefined : undefined,
        });
  ...
  const handleChatGptSignIn = async () => {
    ...
    try {
      const opensable = getOpenSable();
      await opensable.loginOpenAiWithChatGpt();
      const status = await opensable.getOpenAiOauthStatus();
