...
export function ZaiProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      const opensable = getOpenSable();
      const validation = await opensable.validateApiKeyForProvider('zai', apiKey.trim(), {
        region,
      });
      ...
      await opensable.addApiKey('zai', apiKey.trim());

      // Fetch models dynamically
      let fetchedModels: Array<{ id: string; name: string }> | undefined;
      if (providerConfig?.modelsEndpoint) {
        const fetchResult = await opensable.fetchProviderModels('zai', { zaiRegion: region });
