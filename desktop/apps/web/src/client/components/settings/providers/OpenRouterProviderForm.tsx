...
import type { ConnectedProvider, OpenRouterCredentials } from '@opensable/agent-core/common';
import { PROVIDER_META } from '@opensable/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function OpenRouterProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate key
      const validation = await opensable.validateApiKeyForProvider('openrouter', apiKey.trim());
      if (!validation.valid) {
      ...
      // Save key
      await opensable.addApiKey('openrouter', apiKey.trim());

      // Fetch models
      const result = await opensable.fetchOpenRouterModels();
      if (!result.success) {
