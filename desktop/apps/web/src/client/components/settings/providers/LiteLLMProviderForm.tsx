...
import type { ConnectedProvider, LiteLLMCredentials } from '@opensable/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function LiteLLMProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const result = await opensable.testLiteLLMConnection(serverUrl, trimmedKey);
      if (!result.success) {
      ...
      if (trimmedKey) {
        await opensable.addApiKey('litellm', trimmedKey);
      } else {
        // Remove any previously stored key when connecting without one
        await opensable.removeApiKey('litellm');
      }
