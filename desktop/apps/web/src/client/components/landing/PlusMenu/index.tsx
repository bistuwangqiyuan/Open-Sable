...
import type { Skill, McpConnector } from '@opensable/agent-core/common';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
export function PlusMenu({ onSkillSelect, onOpenSettings, disabled }: PlusMenuProps) {
  ...
  const handleRefresh = async () => {
    const opensable = window.opensable;
    if (!opensable || isRefreshing) return;
    setIsRefreshing(true);
    try {
      const [, updatedSkills] = await Promise.all([
        new Promise((resolve) => setTimeout(resolve, 600)),
        opensable.resyncSkills().then(() => opensable.getEnabledSkills()),
      ]);
  ...
  const handleToggleConnector = useCallback(async (id: string, enabled: boolean) => {
    ...
    try {
      await window.opensable.setConnectorEnabled(id, enabled);
