...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  sendFollowUp: async (message: string) => {
    ...
    try {
      ...
      const task = await opensable.resumeSession(sessionId, message, currentTask.id);
  ...
  cancelTask: async () => {
    ...
    if (currentTask) {
      ...
      await opensable.cancelTask(currentTask.id);
      set((state) => ({
  ...
  interruptTask: async () => {
    ...
    if (currentTask && currentTask.status === 'running') {
      ...
      await opensable.interruptTask(currentTask.id);
    }
  },
  ...
  respondToPermission: async (response: PermissionResponse) => {
    ...
    await opensable.respondToPermission(response);
  ...
  loadTasks: async () => {
    const opensable = getOpenSable();
    const tasks = await opensable.listTasks();
    set({ tasks });
  },

  loadTaskById: async (taskId: string) => {
    const opensable = getOpenSable();
    const task = await opensable.getTask(taskId);
    set({ currentTask: task, error: task ? null : 'Task not found' });
  },

  deleteTask: async (taskId: string) => {
    const opensable = getOpenSable();
    await opensable.deleteTask(taskId);
    set((state) => ({
  ...
  clearHistory: async () => {
    const opensable = getOpenSable();
    await opensable.clearTaskHistory();
    set({ tasks: [] });
  },
...
if (typeof window !== 'undefined' && window.opensable) {
  window.opensable.onTaskProgress((progress: unknown) => {
  ...
  window.opensable.onTaskUpdate((event: unknown) => {
  ...
  window.opensable.onTaskSummary?.((data: { taskId: string; summary: string }) => {
    useTaskStore.getState().setTaskSummary(data.taskId, data.summary);
  });

  window.opensable.onTodoUpdate?.((data: { taskId: string; todos: TodoItem[] }) => {
    const state = useTaskStore.getState();
  ...
  window.opensable.onAuthError?.((data: { providerId: string; message: string }) => {
