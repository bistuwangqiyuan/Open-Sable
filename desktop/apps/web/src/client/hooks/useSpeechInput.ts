...
export function useSpeechInput(options: UseSpeechInputOptions = {}): UseSpeechInputState & {
  ...
  useEffect(() => {
    opensable.speechIsConfigured().then((configured) => {
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      const result = await opensable.speechTranscribe(audioData, 'audio/webm');
  ...
  const retry = useCallback(async () => {
    ...
    try {
      ...
      const result = await opensable.speechTranscribe(lastAudioDataRef.current, 'audio/webm');
