...
import { hasAnyReadyProvider } from '@opensable/agent-core/common';
...
export function ExecutionPage() {
  ...
  useEffect(() => {
    opensable.getDebugMode().then(setDebugModeEnabled);
    const unsubscribeDebugMode = opensable.onDebugModeChange?.(({ enabled }) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      opensable.getTodosForTask(id).then((todos) => {
        useTaskStore.getState().setTodos(id, todos);
      });
    }

    const unsubscribeTask = opensable.onTaskUpdate((event) => {
      addTaskUpdate(event);
    ...
    const unsubscribeTaskBatch = opensable.onTaskUpdateBatch?.((event) => {
    ...
    const unsubscribePermission = opensable.onPermissionRequest((request) => {
      setPermissionRequest(request);
    });

    const unsubscribeStatusChange = opensable.onTaskStatusChange?.((data) => {
      if (data.taskId === id) {
        updateTaskStatus(data.taskId, data.status);
      }
    });

    const unsubscribeDebugLog = opensable.onDebugLog((log) => {
  ...
  const handleFollowUp = useCallback(async () => {
    ...
    setFollowUp('');
  }, [followUp, opensable, sendFollowUp]);
