  ...
  describe('buildCreateSkillPrompt', () => {
    it('pins only the deterministic base path for unix paths', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '/Users/test/Library/Application Support/OpenSable/skills/',
        platform: 'darwin',
      });

      expect(prompt).toContain('/skill-creator');
      expect(prompt).toContain(
        'Use this exact base directory: `/Users/test/Library/Application Support/OpenSable/skills`',
      );
      expect(prompt).toContain(
    ...
    it('normalizes windows base path when platform is win32', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: 'C:/Users/Test/AppData/Roaming/OpenSable/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `C:\\Users\\Test\\AppData\\Roaming\\OpenSable\\skills`',
      );
    ...
    it('keeps unix paths with spaces in user home segments', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '/home/Jane Doe/.local/share/OpenSable/skills/',
        platform: 'linux',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `/home/Jane Doe/.local/share/OpenSable/skills`',
      );
    });
    ...
    it('keeps windows paths with spaces in user names', () => {
      const prompt = buildCreateSkillPrompt({
        name: 'Windows Skill',
        description: 'Works on Windows',
        skillsBasePath: 'C:/Users/Jane Doe/AppData/Roaming/OpenSable/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `C:\\Users\\Jane Doe\\AppData\\Roaming\\OpenSable\\skills`',
      );
    });
    ...
    it('preserves tilde-prefixed paths without expansion', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '~/.config/OpenSable/skills/',
        platform: 'linux',
      });

      expect(prompt).toContain('Use this exact base directory: `~/.config/OpenSable/skills`');
    });
    ...
    it('normalizes windows UNC network paths', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '//Server01/OpenSable/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain('Use this exact base directory: `\\\\Server01\\OpenSable\\skills`');
    });
