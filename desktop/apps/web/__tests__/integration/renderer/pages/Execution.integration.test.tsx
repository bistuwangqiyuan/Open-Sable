...
import type { Task, TaskStatus, TaskMessage, PermissionRequest } from '@opensable/agent-core';
import { PROMPT_DEFAULT_MAX_LENGTH } from '@opensable/agent-core/common';
...
// Mock OpenSable icon
vi.mock('/assets/opensable-icon.png', () => ({ default: 'opensable-icon.png' }));
...
describe('Execution Page Integration', () => {
  beforeEach(() => {
    ...
    (window as Window & { opensable: typeof mockOpenSable }).opensable = mockOpenSable;
    // Reset store state
    mockStoreState = {
      currentTask: null,
      loadTaskById: mockLoadTaskById,
  ...
  describe('message display', () => {
    ...
    it('should show thinking indicator when running without tool', () => {
      ...
      expect(
        screen.getByText(/^(Doing|Executing|Running|Handling it|OpenSableing)\.\.\.$/),
      ).toBeInTheDocument();
    });
