...
import type { Task, TaskStatus } from '@opensable/agent-core';
...
describe('Home Page Integration', () => {
  beforeEach(() => {
    ...
    mockOpenSable.getProviderSettings.mockResolvedValue({
      activeProviderId: 'anthropic',
      connectedProviders: {
        anthropic: {
          providerId: 'anthropic',
  ...
  describe('initial render', () => {
    it('should render the main heading', () => {
      ...
      // Assert
      expect(
        screen.getByRole('heading', { name: /what will you opensable today/i }),
      ).toBeInTheDocument();
    });
  ...
  describe('task input integration', () => {
    ...
    it('should check for provider settings before submitting task', async () => {
      ...
      await waitFor(() => {
        expect(mockOpenSable.isE2EMode).toHaveBeenCalled();
      });
    });
    ...
    it('should not submit empty task', async () => {
      ...
      await waitFor(() => {
        expect(mockOpenSable.isE2EMode).not.toHaveBeenCalled();
