 ...
 * Tests store actions with mocked window.opensable API
...
// Mock the opensable module
vi.mock('@/lib/opensable', () => ({
  getOpenSable: () => mockOpenSable,
}));

// Mock window.opensable for global subscriptions
...
describe('taskStore Integration', () => {
  ...
  describe('startTask', () => {
    it('should call startTask API and update state on success', async () => {
      ...
      mockOpenSable.startTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockOpenSable.startTask).toHaveBeenCalledWith(config);
    ...
    it('should set error state on failure', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockOpenSable.startTask.mockRejectedValueOnce(new Error('API Error'));
    ...
    it('should handle non-Error exceptions gracefully', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockOpenSable.startTask.mockRejectedValueOnce('String error');
    ...
    it('should update existing task if same ID', async () => {
      ...
      mockOpenSable.startTask.mockResolvedValueOnce(updatedTask);
  ...
  describe('sendFollowUp', () => {
    ...
    it('should start fresh task for interrupted task without session', async () => {
      ...
      mockOpenSable.startTask.mockResolvedValueOnce(newTask);
      ...
      // Assert
      expect(mockOpenSable.startTask).toHaveBeenCalled();
    });
    ...
    it('should resume session when task has sessionId', async () => {
      ...
      mockOpenSable.resumeSession.mockResolvedValueOnce(resumedTask);
      ...
      // Assert
      expect(mockOpenSable.resumeSession).toHaveBeenCalledWith(
        'session-abc',
        'Continue please',
        'task-123',
      );
    ...
    it('should add user message optimistically', async () => {
      ...
      mockOpenSable.resumeSession.mockResolvedValueOnce(
    ...
    it('should handle resumeSession failure', async () => {
      ...
      mockOpenSable.resumeSession.mockRejectedValueOnce(new Error('Resume failed'));
  ...
  describe('cancelTask', () => {
    it('should call cancelTask API and update status', async () => {
      ...
      mockOpenSable.cancelTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockOpenSable.cancelTask).toHaveBeenCalledWith('task-123');
    ...
    it('should do nothing when no current task', async () => {
      ...
      // Assert
      expect(mockOpenSable.cancelTask).not.toHaveBeenCalled();
    });
  });
  ...
  describe('interruptTask', () => {
    it('should call interruptTask API for running task', async () => {
      ...
      mockOpenSable.interruptTask.mockResolvedValueOnce(undefined);

      // Act
      await useTaskStore.getState().interruptTask();

      // Assert
      expect(mockOpenSable.interruptTask).toHaveBeenCalledWith('task-123');
    });
    ...
    it('should not call API for non-running task', async () => {
      ...
      // Assert
      expect(mockOpenSable.interruptTask).not.toHaveBeenCalled();
    });
  ...
  describe('error state management', () => {
    it('should clear error on successful task start', async () => {
      ...
      mockOpenSable.startTask.mockResolvedValueOnce(createMockTask('task-123'));
  ...
  describe('loadTasks', () => {
    it('should load tasks from API', async () => {
      ...
      mockOpenSable.listTasks.mockResolvedValueOnce(mockTasks);
      ...
      // Assert
      expect(mockOpenSable.listTasks).toHaveBeenCalled();
  ...
  describe('loadTaskById', () => {
    it('should load specific task and set as current', async () => {
      ...
      mockOpenSable.getTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockOpenSable.getTask).toHaveBeenCalledWith('task-123');
      expect(state.currentTask).toEqual(mockTask);
      expect(state.error).toBeNull();
    });
    ...
    it('should set error when task not found', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockOpenSable.getTask.mockResolvedValueOnce(null);
  ...
  describe('deleteTask', () => {
    it('should delete task and remove from list', async () => {
      ...
      mockOpenSable.deleteTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockOpenSable.deleteTask).toHaveBeenCalledWith('task-2');
      expect(state.tasks).toHaveLength(2);
  ...
  describe('clearHistory', () => {
    it('should clear all tasks', async () => {
      ...
      mockOpenSable.clearTaskHistory.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockOpenSable.clearTaskHistory).toHaveBeenCalled();
      expect(state.tasks).toEqual([]);
    });
  });
  ...
  describe('respondToPermission', () => {
    it('should call API and clear permission request', async () => {
      ...
      mockOpenSable.respondToPermission.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockOpenSable.respondToPermission).toHaveBeenCalledWith(response);
      expect(state.permissionRequest).toBeNull();
    });
  });
