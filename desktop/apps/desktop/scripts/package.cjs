...
const opensablePath = path.join(nodeModulesPath, '@opensable');
...
try {
  ...
  for (const pkg of workspacePackages) {
    const pkgPath = path.join(opensablePath, pkg);
  ...
  // Remove empty @opensable directory if it exists
  if (Object.keys(symlinkTargets).length > 0) {
    try {
      fs.rmdirSync(opensablePath);
    } catch {
  ...
  if (packagesToRestore.length > 0) {
    ...
    // Recreate @opensable directory if needed
    if (!fs.existsSync(opensablePath)) {
      fs.mkdirSync(opensablePath, { recursive: true });
    }
