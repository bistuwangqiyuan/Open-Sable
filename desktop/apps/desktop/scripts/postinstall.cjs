...
if (process.env.OPENSABLE_POSTINSTALL_RUNNING) {
  console.log('> Postinstall already running, skipping nested invocation');
  process.exit(0);
}
process.env.OPENSABLE_POSTINSTALL_RUNNING = '1';
...
function runCommand(command, description) {
  ...
  try {
    execSync(command, {
      ...
      env: {
        ...process.env,
        OPENSABLE_POSTINSTALL_RUNNING: '1',
      },
    });
  } catch (error) {
...
const useBundledMcp = process.env.OPENSABLE_BUNDLED_MCP === '1' || process.env.CI === 'true';
