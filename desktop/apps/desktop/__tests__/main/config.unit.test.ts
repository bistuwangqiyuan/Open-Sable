...
describe('config.ts', () => {
  ...
  describe('getDesktopConfig()', () => {
    describe('default configuration', () => {
      it('should return default API URL when OPENSABLE_API_URL is not set', async () => {
        // Arrange
        delete process.env.OPENSABLE_API_URL;
        ...
        // Assert
        expect(config.apiUrl).toBe('http://127.0.0.1:8789');
      });

      it('should return default API URL when OPENSABLE_API_URL is undefined', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = undefined;
    ...
    describe('custom API URL parsing', () => {
      it('should use custom HTTPS API URL from environment', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'https://custom.example.com';
      ...
      it('should use custom HTTP API URL from environment', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'http://localhost:3000';
      ...
      it('should accept URL with path', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'https://api.example.com/v1';
      ...
      it('should accept URL with port', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'https://api.example.com:8443';
      ...
      it('should throw error for invalid URL format', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'not-a-url';
      ...
      it('should throw error for URL without protocol', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'example.com';
    ...
    describe('config caching behavior', () => {
      it('should cache config and return same result on multiple calls', async () => {
        // Arrange
        process.env.OPENSABLE_API_URL = 'https://first.example.com';
        const { getDesktopConfig } = await import('../../src/main/config');
        ...
        process.env.OPENSABLE_API_URL = 'https://second.example.com';
