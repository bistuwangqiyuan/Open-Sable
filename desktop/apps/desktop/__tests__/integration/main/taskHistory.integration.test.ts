...
import type { Task, TaskMessage, TaskStatus } from '@opensable/agent-core';
...
describe('taskHistory Integration', () => {
  ...
  describe('saveTask and getTask', () => {
    it('should save and retrieve a task by ID', async () => {
      // Arrange
      const { saveTask, getTask } = await import('@opensable/agent-core');
    ...
    it('should return null for non-existent task', async () => {
      // Arrange
      const { getTask } = await import('@opensable/agent-core');
  ...
  describe('getTasks', () => {
    it('should return empty array when no tasks exist', async () => {
      // Arrange
      const { getTasks } = await import('@opensable/agent-core');
    ...
    it('should return all saved tasks', async () => {
      // Arrange
      const { saveTask, getTasks } = await import('@opensable/agent-core');
  ...
  describe('updateTaskStatus', () => {
    it('should update task status', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskStatus } = await import('@opensable/agent-core');
  ...
  describe('updateTaskSessionId', () => {
    it('should update session ID for existing task', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskSessionId } = await import('@opensable/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('updateTaskSummary', () => {
    it('should update task summary', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskSummary } = await import('@opensable/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('addTaskMessage', () => {
    it('should add message to existing task', async () => {
      // Arrange
      const { saveTask, getTask, addTaskMessage } = await import('@opensable/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('deleteTask', () => {
    it('should delete task by ID', async () => {
      // Arrange
      const { saveTask, getTask, deleteTask } = await import('@opensable/agent-core');
      saveTask(createMockTask('task-1'));
    ...
    it('should not throw when deleting non-existent task', async () => {
      // Arrange
      const { deleteTask } = await import('@opensable/agent-core');
  ...
  describe('clearHistory', () => {
    it('should remove all tasks', async () => {
      // Arrange
      const { saveTask, getTasks, clearHistory } = await import('@opensable/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('flushPendingTasks', () => {
    it('should be a no-op for SQLite (writes are immediate)', async () => {
      // Arrange
      const { flushPendingTasks } = await import('@opensable/agent-core');
