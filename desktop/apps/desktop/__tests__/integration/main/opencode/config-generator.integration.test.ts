...
// Note: PERMISSION_API_PORT and QUESTION_API_PORT are now imported from @opensable/shared
// by config-generator.ts, so no mock needed here

// Mock @opensable/agent-core (uses SQLite which requires native module)
// Note: generateConfig mock creates real files in temp directory for integration testing
vi.mock('@opensable/agent-core', async () => {
...
Use AskUserQuestion tool for user interaction.`,
        ...
        return {
          configPath,
          systemPrompt: config.agent.opensable.prompt,
    ...
    OPENSABLE_AGENT_NAME: 'opensable',
...
describe('OpenCode Config Generator Integration', () => {
  ...
  describe('generateOpenCodeConfig()', () => {
    ...
    it('should write config file with correct structure', async () => {
      ...
      expect(config.default_agent).toBe('opensable');
    ...
    it('should include opensable agent configuration', async () => {
      // Act
      const { generateOpenCodeConfig } = await import('@main/opencode/config-generator');
      const configPath = await generateOpenCodeConfig();
      ...
      const agent = config.agent['opensable'];
    ...
    it('should include platform-specific environment instructions', async () => {
      ...
      // Assert
      const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      const prompt = config.agent['opensable'].prompt;
    ...
    it('should include enabled connected connectors with access tokens', async () => {
      ...
      const { generateConfig } = await import('@opensable/agent-core');
  ...
  describe('OPENSABLE_AGENT_NAME Export', () => {
    it('should export the agent name constant', async () => {
      // Act
      const { OPENSABLE_AGENT_NAME } = await import('@main/opencode/config-generator');

      // Assert
      expect(OPENSABLE_AGENT_NAME).toBe('opensable');
    });
  });
