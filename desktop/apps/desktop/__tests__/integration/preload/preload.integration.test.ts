...
let capturedOpenSableAPI: Record<string, unknown> = {};
let capturedOpenSableShell: Record<string, unknown> = {};

describe('Preload Script Integration', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    capturedOpenSableAPI = {};
    capturedOpenSableShell = {};
    ...
    mockExposeInMainWorld.mockImplementation((name: string, api: unknown) => {
      if (name === 'opensable') {
        capturedOpenSableAPI = api as Record<string, unknown>;
      } else if (name === 'opensableShell') {
        capturedOpenSableShell = api as Record<string, unknown>;
      }
    });
  ...
  describe('API Exposure', () => {
    it('should expose opensable API via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('opensable', expect.any(Object));
      expect(capturedOpenSableAPI).toBeDefined();
    });

    it('should expose opensableShell info via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('opensableShell', expect.any(Object));
      expect(capturedOpenSableShell).toBeDefined();
    });

    it('should expose shell info with isElectron=true', () => {
      expect(capturedOpenSableShell.isElectron).toBe(true);
    });

    it('should expose shell info with platform', () => {
      expect(capturedOpenSableShell.platform).toBe(process.platform);
    });

    it('should expose shell info with version matching package.json', () => {
      expect(capturedOpenSableShell.version).toBe(pkg.version);
    });
  });
  ...
  describe('IPC Method Invocations', () => {
    describe('App Info', () => {
      it('getVersion should invoke app:version', async () => {
        await (capturedOpenSableAPI.getVersion as () => Promise<string>)();
        expect(mockInvoke).toHaveBeenCalledWith('app:version');
      });

      it('getPlatform should invoke app:platform', async () => {
        await (capturedOpenSableAPI.getPlatform as () => Promise<string>)();
    ...
    describe('Shell Operations', () => {
      it('openExternal should invoke shell:open-external with URL', async () => {
        ...
        await (capturedOpenSableAPI.openExternal as (url: string) => Promise<void>)(url);
    ...
    describe('Task Operations', () => {
      it('startTask should invoke task:start with config', async () => {
        ...
        await (
          capturedOpenSableAPI.startTask as (config: { description: string }) => Promise<unknown>
        )(config);
        expect(mockInvoke).toHaveBeenCalledWith('task:start', config);
      });

      it('cancelTask should invoke task:cancel with taskId', async () => {
        await (capturedOpenSableAPI.cancelTask as (taskId: string) => Promise<void>)('task_123');
      ...
      it('interruptTask should invoke task:interrupt with taskId', async () => {
        await (capturedOpenSableAPI.interruptTask as (taskId: string) => Promise<void>)(
          'task_123',
        );
    ...
    describe('Permission Operations', () => {
      it('respondToPermission should invoke permission:respond', async () => {
        ...
        await (
          capturedOpenSableAPI.respondToPermission as (r: {
            taskId: string;
            allowed: boolean;
          }) => Promise<void>
        )(response);
    ...
    describe('Session Operations', () => {
      it('resumeSession should invoke session:resume', async () => {
        await (
          capturedOpenSableAPI.resumeSession as (
            s: string,
            p: string,
            t?: string,
          ) => Promise<unknown>
    ...
    describe('Settings Operations', () => {
      it('getDebugMode should invoke settings:debug-mode', async () => {
        await (capturedOpenSableAPI.getDebugMode as () => Promise<boolean>)();
        expect(mockInvoke).toHaveBeenCalledWith('settings:debug-mode');
      });

      it('setDebugMode should invoke settings:set-debug-mode', async () => {
        await (capturedOpenSableAPI.setDebugMode as (enabled: boolean) => Promise<void>)(true);
      ...
      it('getAppSettings should invoke settings:app-settings', async () => {
        await (capturedOpenSableAPI.getAppSettings as () => Promise<unknown>)();
    ...
    describe('Onboarding Operations', () => {
      it('getOnboardingComplete should invoke onboarding:complete', async () => {
        await (capturedOpenSableAPI.getOnboardingComplete as () => Promise<boolean>)();
      ...
      it('setOnboardingComplete should invoke onboarding:set-complete', async () => {
        await (capturedOpenSableAPI.setOnboardingComplete as (c: boolean) => Promise<void>)(true);
    ...
    describe('Model Operations', () => {
      it('getSelectedModel should invoke model:get', async () => {
        await (capturedOpenSableAPI.getSelectedModel as () => Promise<unknown>)();
      ...
      it('setSelectedModel should invoke model:set', async () => {
        ...
        await (
          capturedOpenSableAPI.setSelectedModel as (m: {
            provider: string;
            model: string;
          }) => Promise<void>
        )(model);
    ...
    describe('Logging Operations', () => {
      it('logEvent should invoke log:event', async () => {
        ...
        await (capturedOpenSableAPI.logEvent as (p: unknown) => Promise<unknown>)(payload);
    ...
    describe('Skills Operations', () => {
      it('getUserSkillsPath should invoke skills:get-user-skills-path', async () => {
        await (capturedOpenSableAPI.getUserSkillsPath as () => Promise<string>)();
  ...
  describe('Event Subscriptions', () => {
    it('onTaskUpdate should subscribe to task:update', () => {
      ...
      (capturedOpenSableAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskUpdate should return unsubscribe function', () => {
      const callback = vi.fn();
      const unsubscribe = (
        capturedOpenSableAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void
      )(callback);
      unsubscribe();
    ...
    it('onTaskUpdateBatch should subscribe to task:update:batch', () => {
      const callback = vi.fn();
      (capturedOpenSableAPI.onTaskUpdateBatch as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onPermissionRequest should subscribe to permission:request', () => {
      const callback = vi.fn();
      (capturedOpenSableAPI.onPermissionRequest as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onTaskProgress should subscribe to task:progress', () => {
      const callback = vi.fn();
      (capturedOpenSableAPI.onTaskProgress as (cb: (e: unknown) => void) => () => void)(callback);
      expect(mockOn).toHaveBeenCalledWith('task:progress', expect.any(Function));
    });

    it('onDebugLog should subscribe to debug:log', () => {
      const callback = vi.fn();
      (capturedOpenSableAPI.onDebugLog as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskStatusChange should subscribe to task:status-change', () => {
      const callback = vi.fn();
      (capturedOpenSableAPI.onTaskStatusChange as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
