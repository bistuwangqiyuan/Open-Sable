...
## Saving Skills in OpenSable

**IMPORTANT:** When creating skills in OpenSable, you can ONLY create "custom" skills. You CANNOT create "official" skills - those are bundled with the app and managed by the OpenSable team.
...
Skills must be saved to the OpenSable user data directory under a `skills` folder:

**macOS:** `~/Library/Application Support/OpenSable/skills/<skill-name>/SKILL.md`
**Windows:** `%APPDATA%\OpenSable\skills\<skill-name>\SKILL.md`
**Linux:** `~/.config/OpenSable/skills/<skill-name>/SKILL.md`
...
1. **Use the base path from the task prompt** when present.
   - If no path is provided, detect the OS and use:
     - macOS: `~/Library/Application Support/OpenSable/skills/`
     - Windows: `%APPDATA%\OpenSable\skills\`
     - Linux: `~/.config/OpenSable/skills/`
...
5. **The skill is automatically detected** - OpenSable scans this directory on startup and syncs new skills to its database. The skill will appear in Settings > Skills as a "Custom" skill.

### Skill Frontmatter Rules

For custom skills in OpenSable:
