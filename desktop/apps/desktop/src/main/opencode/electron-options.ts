...
import type { TaskManagerOptions, TaskCallbacks } from '@opensable/agent-core';
import type { TaskConfig } from '@opensable/agent-core';
import { DEV_BROWSER_PORT } from '@opensable/agent-core';
import {
  getAzureEntraToken,
...
import { getModelDisplayName } from '@opensable/agent-core';
import type {
  ...
  if (!bundledNode) {
    throw new Error(
      '[OpenCode CLI] Bundled Node.js path is missing. ' +
        'Run "pnpm -F @opensable/desktop download:nodejs" and rebuild before launching.',
    );
  }
