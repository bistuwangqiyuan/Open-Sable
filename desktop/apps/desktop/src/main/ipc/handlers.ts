...
import { createTaskId, createMessageId } from '@opensable/agent-core';
import {
  storeApiKey,
  getApiKey,
  deleteApiKey,
...
import { getOpenAiOauthStatus } from '@opensable/agent-core';
...
export function registerIPCHandlers(): void {
    ...
    async (
      ...
      if (STANDARD_VALIDATION_PROVIDERS.has(provider)) {
        ...
        const result = await validateApiKey(
          provider as import('@opensable/agent-core').ProviderType,
          sanitizedKey,
          {
            timeout: API_KEY_VALIDATION_TIMEOUT_MS,
            baseUrl:
            ...
            zaiRegion:
              provider === 'zai'
                ? (options?.region as import('@opensable/agent-core').ZaiRegion) ||
                  'international'
                : undefined,
          },
        );
  ...
  handle(
    ...
    async (
      ...
      if (providerId === 'zai' && options?.zaiRegion) {
        const region = options.zaiRegion as import('@opensable/agent-core').ZaiRegion;
  ...
  handle('logs:export', async (event: IpcMainInvokeEvent) => {
    ...
    const defaultFilename = `opensable-logs-${timestamp}.txt`;
    ...
    try {
      if (fs.existsSync(logPath)) {
        ...
        const header = `OpenSable Application Logs\nExported: ${new Date().toISOString()}\nLog Directory: ${logDir}\n\nNo logs recorded yet.\n`;
  ...
  handle('connectors:start-oauth', async (_event, connectorId: string) => {
    ...
    const authUrl = buildAuthorizationUrl({
      ...
      redirectUri: 'opensable://callback/mcp',
