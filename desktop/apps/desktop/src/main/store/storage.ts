...
import { createStorage, type StorageAPI } from '@opensable/agent-core';
// Deep import for legacy migration only — getDatabase is intentionally not part of StorageAPI
import { getDatabase as coreGetDatabase } from '@opensable/agent-core/storage/database';
...
export function getDatabasePath(): string {
  const dbName = app.isPackaged ? 'opensable.db' : 'opensable-dev.db';
