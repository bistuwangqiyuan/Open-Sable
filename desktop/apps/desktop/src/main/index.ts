...
const APP_DATA_NAME = 'OpenSable';
app.setPath('userData', path.join(app.getPath('appData'), APP_DATA_NAME));

if (process.platform === 'win32') {
  app.setAppUserModelId('ai.opensable.desktop');
}

import { registerIPCHandlers } from './ipc/handlers';
import { FutureSchemaError } from '@opensable/agent-core';
import { initThoughtStreamApi, startThoughtStreamServer } from './thought-stream-api';
import type { ProviderId } from '@opensable/agent-core';
...
const ROUTER_URL = process.env.OPENSABLE_ROUTER_URL;
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, commandLine) => {
    if (mainWindow) {
      ...
      if (process.platform === 'win32') {
        const protocolUrl = commandLine.find((arg) => arg.startsWith('opensable://'));
        if (protocolUrl) {
          console.log('[Main] Received protocol URL from second-instance:', protocolUrl);
          if (protocolUrl.startsWith('opensable://callback/mcp')) {
            mainWindow.webContents.send('auth:mcp-callback', protocolUrl);
          } else if (protocolUrl.startsWith('opensable://callback')) {
    ...
    try {
      ...
      if (err instanceof FutureSchemaError) {
        await dialog.showMessageBox({
          ...
          message: `This data was created by a newer version of OpenSable (schema v${err.storedVersion}).`,
          detail: `Your app supports up to schema v${err.appVersion}. Please update OpenSable to continue.`,
          buttons: ['Quit'],
        });
        app.quit();
        return;
...
if (process.platform === 'win32' && !app.isPackaged) {
  app.setAsDefaultProtocolClient('opensable', process.execPath, [path.resolve(process.argv[1])]);
} else {
  app.setAsDefaultProtocolClient('opensable');
}

function handleProtocolUrlFromArgs(): void {
  if (process.platform === 'win32') {
    const protocolUrl = process.argv.find((arg) => arg.startsWith('opensable://'));
    if (protocolUrl) {
      app.whenReady().then(() => {
        setTimeout(() => {
...
app.on('open-url', (event, url) => {
  ...
  if (url.startsWith('opensable://callback/mcp')) {
    mainWindow?.webContents?.send('auth:mcp-callback', url);
  } else if (url.startsWith('opensable://callback')) {
