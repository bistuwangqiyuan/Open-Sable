...
if (hostNodeTarget) {
  if (!isHostNodeRuntimeAvailable(hostNodeTarget)) {
    ...
    runPnpm(
      ['-F', '@opensable/desktop', 'download:nodejs', `--platform=${hostNodeTarget}`],
...
if (needsAgentCoreBuild) {
  console.log('Missing @opensable/agent-core build outputs:');
  ...
  runPnpm(['-F', '@opensable/agent-core', 'build'], 'Building @opensable/agent-core...');

  missingOutputs = [...outputPaths].filter(
    (relativeOutputPath) => !fs.existsSync(path.join(agentCoreDir, relativeOutputPath)),
  );
  if (missingOutputs.length > 0) {
    console.error('Failed to produce required @opensable/agent-core outputs:');
    for (const missingOutput of missingOutputs) {
...
if (missingMcpDistOutputs.length > 0) {
  ...
  runPnpm(
    ['-F', '@opensable/desktop', 'build:mcp-tools:dev'],
...
console.log('✓ @opensable/agent-core and MCP dist outputs found');
process.exit(0);
