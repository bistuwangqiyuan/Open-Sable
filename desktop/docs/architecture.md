...
packages/agent-core/     # Core business logic, shared types, MCP tools (published as @opensable/agent-core)
```

## Package Dependency Graph

```
@opensable/agent-core (business logic, types, constants, storage, MCP tools)
        ↑
@opensable/desktop (Electron app)
```
...
- `index.ts` - Electron bootstrap, single-instance enforcement, `opensable://` protocol handler
...
- Exposes `window.opensable` API via `contextBridge`
- Provides `window.opensableShell` for shell metadata
...
- `lib/opensable.ts` - Typed wrapper for the IPC API
...
App data is stored in SQLite (`opensable.db` in production, `opensable-dev.db` in development) located in the user data directory.
...
import { getNpxPath, getBundledNodePaths } from '@opensable/agent-core/utils';
